...
router.post('/generate-video-ad', async (req, res) => {
  try {
    ...
    // 1. Search Pexels for relevant ecomm videos (get 6 for choice)
    let videoClips = [];
    try {
    ...
    // 2. Pick up to 3 short relevant SD clips
    let files = [];
    ...
    // 3. Download locally (with timeout and size limit)
    ...
    // 6. Concat up to 3 clips, overlay voice, 18s max
    ...
    // FFmpeg: concat, add audio, enforce max 18s (trim longer, pad if short)
    const tempConcat = `${outPath}.temp.mp4`;
    await exec(`${ffmpegPath} -y -f concat -safe 0 -i "${listPath}" -c copy "${tempConcat}"`);
    await exec(`${ffmpegPath} -y -i "${tempConcat}" -i "${ttsPath}" -map 0:v:0 -map 1:a:0 -shortest -t 18 -c:v libx264 -c:a aac -b:a 192k -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" "${outPath}"`);
