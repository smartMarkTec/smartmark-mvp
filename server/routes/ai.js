...
/* ------------- Lightweight security & rate-limiting (per-router) ------------- */
const { secureHeaders, basicRateLimit } = require('../middleware/security');
router.use(secureHeaders());                                // secure defaults for all AI endpoints
router.use(basicRateLimit({ windowMs: 15 * 60 * 1000, max: 120 })); // general cap
const heavyLimiter = basicRateLimit({ windowMs: 60 * 60 * 1000, max: 20 }); // generation-heavy
...
function getUserToken(req) {
  // Prefer Authorization: Bearer, then server session, then request body token, then (last resort) shared store.
...
async function uploadVideoToAdAccount(adAccountId, userAccessToken, fileUrl, name = 'SmartMark Video', description = 'Generated by SmartMark') {
  const id = String(adAccountId || '').replace(/^act_/, '').replace(/\D/g, ''); // sanitize
...
/* -- Build the overlaid JPG using the SVG above -- */
...
function finalizeScriptWithSingleCTA(text, chosenCTA) {
  const CTA_PLAIN = ['SHOP NOW','BUY NOW','CHECK US OUT','VISIT US','TAKE A LOOK','LEARN MORE','GET STARTED'];
