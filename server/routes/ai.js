...
async function buildOverlayImage ({ imageUrl, headlineHint = '', ctaHint = '', seed = '', fallbackHeadline = 'SHOP', answers = {}, category = 'generic' }) {
  ...
  const overlaySVG = Buffer.from(
    `<svg width="${W}" height="${H}" xmlns="http://www.w3.org/2000/svg">${svgOverlayCreative({
      W, H, title, subline, cta, prefer: analysis.darkerSide, preferBand: analysis.darkerBand, brandColor: analysis.brandColor, choose: tpl, sale
    })}</svg>`
  );
  const outDir = ensureGeneratedDir();
  const file = `${uuidv4()}.jpg`;
  await base.composite([{ input: overlaySVG, top: 0, left: 0 }]).jpeg({ quality: 93, chromaSubsampling: '4:4:4', mozjpeg: true }).toFile(path.join(outDir, file));
  maybeGC();
...
Output ONLY the script text.`;
    ...
    try {
      if (variations[0] && req.body.fbAdAccountId && token) {
        const up = await uploadVideoToAdAccount(req.body.fbAdAccountId, token, variations[0].absoluteUrl, 'SmartMark Generated Video', 'Generated by SmartMark');
  ...
  try {
    ...
    const makeOne = async (baseUrl, seed) => {
      ...
      try {
        ...
        await saveAsset({ req, kind: 'image', url: publicUrl, absoluteUrl, meta: { keyword, overlayText: ctaHint, headlineHint, category, sale } });
        return publicUrl;
      } catch {
        await saveAsset({ req, kind: 'image', url: baseUrl, absoluteUrl: baseUrl, meta: { keyword, overlayText: ctaHint, headlineHint, raw: true, category, sale } });
        return baseUrl;
      }
    };
