...
function buildAdEditPromptFromAnswers(a = {}, craftedCopy = {}, { logoFound = false } = {}) {
  ...
  return [
    ...
    ``,
    `Text layout: all text must stay entirely within the top 80% of the image canvas — the bottom 20% must be completely clear of text. This is a hard boundary: no element may touch, approach, or cross it — not the CTA, not the website URL, not the offer line, not the last word of any text block. If the full text stack cannot fit above that boundary at a legible size, simplify rather than overflow: first drop the supporting body text; if still too tall, shorten the website to the bare domain name; last resort reduce font size — but never push any element past the 80% mark and never crop, cut off, or partially hide any word, letter, or character. Use concise copy: a short headline, a brief one-line subline only if it fits cleanly, and a tight CTA. Avoid dense text blocks. Text must be fully legible at social-feed viewing sizes.`,
    ``,
