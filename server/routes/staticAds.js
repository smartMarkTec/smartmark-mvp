...
Typography: use a clean, minimal overlay — the image does not need to show every provided detail. Prioritize the headline and CTA. Supporting text, phone number, and website URL are optional; include them only if they fit naturally without making the design feel crowded or template-like. A well-designed ad often shows just a headline, CTA, and one or two supporting elements. Keep any displayed text fully visible inside the image with a comfortable bottom margin — no word or line should touch or cross the lower edge. Use clean, well-spaced type with a clear visual hierarchy.
...
async function compositeLogoOntoAd(adBuf, logoBuf) {
  ...
  try {
    ...
    const pad = Math.round(adW * 0.045);
