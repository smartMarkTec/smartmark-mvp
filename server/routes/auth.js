...
const FB_SCOPES = [
  'pages_manage_engagement','pages_manage_metadata','pages_manage_posts','pages_read_engagement','pages_read_user_content',
  'pages_show_list','public_profile','read_insights','business_management','ads_management','ads_read'
];
...
router.get('/facebook', (req, res) => {
  ...
  const fbUrl = `https://www.facebook.com/v18.0/dialog/oauth?client_id=${FACEBOOK_APP_ID}` +
...
router.get('/facebook/adaccounts', async (req, res) => {
  ...
  try {
    const me = await axios.get(`https://graph.facebook.com/v18.0/me/adaccounts`, {
...
router.post('/signup', async (req, res) => {
  ...
  if (!username || !email || !cashtag || !password) return res.status(400).json({ error: 'All fields required' });
  await db.read();
...
/* Launch campaign — (your existing logic unchanged except route name space) */
router.post('/facebook/adaccount/:accountId/launch-campaign', async (req, res) => {
  ...
  try {
    ...
    // Campaign
    const campaignRes = await axios.post(`https://graph.facebook.com/v18.0/act_${accountId}/campaigns`, {
      name: campaignName, objective: 'OUTCOME_TRAFFIC', status: 'ACTIVE', special_ad_categories: []
    ...
    if (wantImage) {
      const { data } = await axios.post(`https://graph.facebook.com/v18.0/act_${accountId}/adsets`, {
        name: `${campaignName} (Image) - ${new Date().toISOString()}`,
        campaign_id: campaignId, daily_budget: perAdsetBudgetCents,
        billing_event: 'IMPRESSIONS', optimization_goal: 'LINK_CLICKS', bid_strategy: 'LOWEST_COST_WITHOUT_CAP',
        status: 'ACTIVE', start_time: new Date(Date.now() + 60 * 1000).toISOString(),
        targeting: {
          ...targeting,
    ...
    async function ensureVideoIdByIndex(idx) {
      ...
      const up = await axios.post(`https://graph.facebook.com/v23.0/act_${accountId}/advideos`, formd, {
    ...
    if (wantImage && imageAdSetId) {
      for (let i = 0; i < needImg; i++) {
        ...
        const cr = await axios.post(`https://graph.facebook.com/v18.0/act_${accountId}/adcreatives`, {
          name: `${campaignName} (Image v${i + 1})`,
          object_story_spec: {
            page_id: pageId,
            link_data: { message: adCopy || '', link: form.url || 'https://your-smartmark-site.com', image_hash: hash, description: form.description || '' }
          }
        }, { params: { access_token: userToken } });
        const ad = await axios.post(`https://graph.facebook.com/v18.0/act_${accountId}/ads`, {
    ...
    if (wantVideo && videoAdSetId) {
      for (let i = 0; i < needVid; i++) {
        ...
        const video_data = {
          video_id, message: adCopy || '', title: campaignName,
    ...
    res.json({ success: true, campaignId, adSetIds: [imageAdSetId, videoAdSetId].filter(Boolean), adIds, variantPlan: plan, campaignStatus: 'ACTIVE' });
  } catch (err) {
...
/* Inspect existing campaigns */
router.get('/facebook/adaccount/:accountId/campaigns', async (req, res) => {
  const userToken = userTokens['singleton'];
  const { accountId } = req.params;
  if (!userToken) return res.status(401).json({ error: 'Not authenticated with Facebook' });
  try {
    const response = await axios.get(`https://graph.facebook.com/v18.0/act_${accountId}/campaigns`, {
...
router.get('/facebook/campaign/:campaignId/ads', async (req, res) => {
  ...
  if (!userToken) return res.status(401).json({ error: 'Not authenticated with Facebook' });
  try {
    const response = await axios.get(`https://graph.facebook.com/v18.0/${campaignId}/ads`, {
      params: { access_token: userToken, fields: 'id,name,status,effective_status,adset_id,created_time', limit: 200 }
    });
    res.json(response.data);
  } catch (err) {
    res.status(500).json({ error: err.response?.data?.error?.message || 'Failed to list ads.' });
  }
});
...
router.get('/facebook/ad/:adId/insights', async (req, res) => {
  ...
  if (!userToken) return res.status(401).json({ error: 'Not authenticated with Facebook' });
  try {
    const response = await axios.get(`https://graph.facebook.com/v18.0/${adId}/insights`, {
      params: { access_token: userToken, fields: 'impressions,clicks,spend,ctr,cpm', date_preset: 'maximum' }
    });
    res.json(response.data);
  } catch (err) {
...
/* Admin: override global STOP_RULES used by scheduler */
router.post('/admin/set-policy-stop-rules', (req, res) => {
  const body = req.body || {};
  if (typeof body !== 'object') return res.status(400).json({ error: 'Invalid body' });
  ['MIN_SPEND_PER_AD','MIN_IMPRESSIONS_PER_AD','MIN_CLICKS_PER_AD','MAX_TEST_HOURS'].forEach(k => {
    if (Object.prototype.hasOwnProperty.call(body, k)) policy.STOP_RULES[k] = Number(body[k]);
  });
