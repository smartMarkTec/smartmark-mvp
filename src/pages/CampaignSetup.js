...
const handleSaveCopyEdit = async () => {
  ...
  const trimmedHeadline = String(copyEditHeadline || "").trim();

  // Draft (pre-launch) campaigns have no live Meta campaign to PATCH yet — the
  // launch payload reads copy straight from previewCopy / draftCreatives.creativeSet[0],
  // so just update those directly instead of calling the Meta-backed copy endpoint.
  if (!selectedCampaignId || selectedCampaignId === "__DRAFT__") {
    setPreviewCopy((prev) => ({ ...(prev || {}), headline: trimmedHeadline || prev?.headline || "", body: trimmedText }));
    if (draftCreatives?.creativeSet?.length === 1) {
      const updatedDraft = {
        ...draftCreatives,
        creativeSet: [{ ...draftCreatives.creativeSet[0], headline: trimmedHeadline || draftCreatives.creativeSet[0].headline, body: trimmedText }],
      };
...
const handleDeleteCampaign = async (campaignId) => {
  ...
  const handleNewCampaign = () => {
    ...
    // Clear stale draft state and open the AI Agent tab for new campaign creation
    purgeDraftArtifactsEverywhere();
    setDraftDisabled(resolvedUser, false);
    const freshCtx = `${Date.now()}|new||`;
    setActiveCtx(freshCtx, resolvedUser);
    setDraftCreatives({ images: [], mediaSelection: "image", creativeSet: null, creativeTestCount: 1 });
              ...
              <>
                ...
                >
                  <div>
                    ...
                    <div style={{ color: "#64748b", fontWeight: 700, fontSize: 13 }}>
                      {isDraftView
                        ? "These creatives were generated by AI Ad Agent and ready for launch."
                  ...
                  <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 8 }}>
                      ...
                      <div style={{ position: "relative" }}>
                        ...
                        {creativeMenuOpen && (
                          ...
                          >
                            ...
                            {/* Hidden for multi-ad draft tests — edit copy directly on each card instead. */}
                            {!(isDraftView && draftCreatives?.creativeSet?.length > 1) && (
                              <>
                        ...
                        <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
                          ...
                          <button
                            ...
                            onClick={() => {
                              ...
                              // Creative replace now goes to AI Agent tab (FormPage removed from flow)
                              setSetupTab("ai-agent");
                            }}
                    ...
                    {!isDraftView && (() => {
                      const _dl = Array.isArray(selectedCampaignCreatives?.pendingChallengerDrafts) ? selectedCampaignCreatives.pendingChallengerDrafts : [];
                      console.log("[DRAFTS_RENDERING_IN_CREATIVES]", { selectedCampaignId, draftCount: _dl.length });
                      return null;
                    })()}
                    {!isDraftView && Array.isArray(selectedCampaignCreatives?.pendingChallengerDrafts) && selectedCampaignCreatives.pendingChallengerDrafts.length > 0 && (
                      <div style={{ width: "100%", marginTop: 16 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 10 }}>
                          <div style={{ fontWeight: 800, fontSize: 13, color: "#7c3aed" }}>Drafts pending review</div>
                          ...
                          <span style={{ fontSize: 11, color: "#94a3b8", fontStyle: "italic" }}>Not yet on Meta · review before publishing</span>
                        </div>
                        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                          {selectedCampaignCreatives.pendingChallengerDrafts.map((draft, di) => {
                            const imgSrc = draft.imageUrl ? toAbsoluteMedia(draft.imageUrl) : null;
                            return (
                              <div
                              ...
                              >
                                ...
                                {imgSrc ? (
                                  <img
                                    src={imgSrc}
                                    alt={draft.name}
                                    style={{ width: "100%", borderRadius: 8, aspectRatio: "1.9/1", objectFit: "cover", border: "1px solid #e9d5ff" }}
                                ...
                                <div style={{ fontSize: 12, fontWeight: 700, color: "#3b0764", lineHeight: 1.3 }}>{draft.headline}</div>
                                {draft.body && (
                                ...
                                <div style={{ fontSize: 10, color: "#94a3b8" }}>
                                  CTA: {draft.cta || "—"}
                                  {draft.link && <> · <a href={draft.link} target="_blank" rel="noreferrer" style={{ color: "#7c3aed" }}>Landing page ↗</a></>}
                                </div>
                              </div>
                            );
                          })}
                    ...
                    {/* Hidden when a multi-ad draft test is active — those cards have their own
                        per-card "Edit" affordance below, which is the more precise control. */}
                    {!(isDraftView && draftCreatives?.creativeSet?.length > 1) && !selectedLiveCampaign?.smArchived && (
                      <button
                        type="button"
                        onClick={() => {
                          setCopyEditPrimaryText(
                ...
                >
                  ...
                  >
                    {isDraftView
                      ? "These draft creatives were generated by the AI Ad Agent and should appear here automatically before launch."
                      : isTesting
...
{setupTab === "ai-agent" && (
  <InlineAdAgent
    ...
    onSetBudget={(b) => {
      ...
      if (Number.isFinite(num) && num > 0) setBudget(String(num));
    }}
    onSetCampaignName={(n) => {
